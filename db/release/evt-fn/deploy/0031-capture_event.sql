...
BEGIN;
  ...
  BEGIN
    ...
    _idempotency_hash := public.digest(_evt_capture_info.params::text, 'md5'); 
    ...
    if _evt.id is null then
      ...
      insert into evt.evt_consumption(
        evt_id
      ...
      select
        ...
        ,ets.id
        ,'Captured'
      from evt.evt_type_subscriber ets
      where ets.evt_type = _evt.evt_type
      ;
  ...
  --||--
  GRANT execute ON FUNCTION evt_fn.capture_event(evt.evt_capture_info) TO app_demon;
